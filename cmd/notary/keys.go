...
func (k *keyCommander) keysRotate(cmd *cobra.Command, args []string) error {
	...
	if k.rotateKeyServerManaged {
		if rotateKeyRole != data.CanonicalSnapshotRole && rotateKeyRole != data.CanonicalTimestampRole {
			return fmt.Errorf(
				"remote signing/key management is only supported for the snapshot and timestamp keys")
		}
	} else {
		if rotateKeyRole == data.CanonicalTimestampRole {
			return fmt.Errorf("local signing/key management is not supported for timestamp keys")
		}
	}
