...
//Testing that Export exports correctly encrypted information given a certain passphrase
func TestExportFromNativeStore(t *testing.T) {
	myKeyNativeStore, err := NewKeyNativeStore(passphrase.ConstantRetriever(testingPassphrase))
	require.NoError(t, err)
	if nativeStoreNotDefined(myKeyNativeStore) {
		t.Skip("You need to make the native store in docker-credential-helpers and point your path to the binary")
	}
	privKey, _ := GenerateECDSAKey(rand.Reader)
	myKeyInfo := KeyInfo{
		Gun:  "http://example.com/collections",
		Role: "Snapshot",
	}
	...
	encryptedSecret, err := myKeyNativeStore.ExportKey(privKey.ID())
	require.NoError(t, err)
	// decrypt to test the encryption
	gotKey, err := ParsePEMPrivateKey(encryptedSecret, testingPassphrase)
...
//Testing that export behaves gracefully on a key that doesn't exist
func TestExportFromNativeStoreNoPanic(t *testing.T) {
	myKeyNativeStore, err := NewKeyNativeStore(passphrase.ConstantRetriever(testingPassphrase))
	require.NoError(t, err)
	if nativeStoreNotDefined(myKeyNativeStore) {
		t.Skip("You need to make the native store in docker-credential-helpers and point your path to the binary")
	}
	encryptedSecret, err := myKeyNativeStore.ExportKey("randomkeythatshouldnotexistinnativestore(i hope)")
	require.Error(t, err)
	var expected []byte
	expected = nil
